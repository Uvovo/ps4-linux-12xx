...
static void bpcie_msi_mask(struct irq_data *data)
{
	...
	struct pci_dev *pdev = msi_desc_to_pci_dev(desc);
	u32 msi_mask = desc->masked;
	u32 msi_allocated = desc->nvec_used;
...
static void bpcie_msi_calc_mask(struct irq_data *data) {
  ...
  if ( num_of_alloc_messages > 0 )
  {
    ...
    do
    {
      unit_no = LOWORD(ivars->conf.pd_unit) + count++;
...
struct irq_domain *bpcie_create_irq_domain(struct bpcie_dev *sc, struct pci_dev *pdev)
{
