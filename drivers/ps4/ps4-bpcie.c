						  ...
						  struct pci_dev *pdev)
{
	...
	return msi_create_irq_domain(NULL, &bpcie_msi_domain_info,
				     x86_vector_domain);
}
...
int bpcie_sata_phy_init(struct pci_dev *pdev, void __iomem *ahci_mmio)
{
	...
	iowrite32(ioread32(ahci_mmio + 0x2054) &
		  (0xFFFFF07F | (tune1 << 7)), ahci_mmio + 0x2054);
	iowrite32(ioread32(ahci_mmio + 0x201C) | 0x4, ahci_mmio + 0x201C);
	iowrite32(ioread32(ahci_mmio + 0x2078) &
		  (0xFFFFFE0F | (tune2 << 4)), ahci_mmio + 0x2078);
