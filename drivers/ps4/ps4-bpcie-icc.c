...
static void __maybe_unused bpcie_init_usb(struct bpcie_dev *sc, int usb_no)
{
	u32 value_to_write;
	u32 offset;
	void __iomem *addr;
...
static void __maybe_unused resetUsbPort(void)
{
	u8 on = 1;
	u8 resp[20];
	int ret;
...
static void __maybe_unused resetBtWlan(void)
{
	u8 on = 3;
	u8 resp[20];
	int ret;
