...
static DEFINE_MUTEX(bpcie_icc_mutex);

#define REQUEST (sc->icc.spm + BPCIE_SPM_ICC_REQUEST)
#define REPLY (sc->icc.spm + BPCIE_SPM_ICC_REPLY)

static int bpcie_icc_timeout = 15;
static void *ioctl_tmp_buf;
...
static void handle_message(struct bpcie_dev *sc)
{
	...
	off = ICC_HDR_SIZE;
	copy_size = min(sc->icc.reply_length, (int)(msg.length - off));
...
static int _bpcie_icc_cmd(struct bpcie_dev *sc, u8 major, u16 minor,
			  const void *data, u16 length, void *reply,
			  u16 reply_length, bool intr)
{
	int ret;
	...
	if (intr)
		ret = wait_event_interruptible_timeout(sc->icc.wq,
				!sc->icc.reply_pending,
				HZ * bpcie_icc_timeout);
	else
		  ...
		  void *reply, u16 reply_length)
{
	...
	ret = _bpcie_icc_cmd(icc_sc, major, minor, data, length,
			     reply, reply_length, false);
out_unlock:
	mutex_unlock(&bpcie_icc_mutex);
	return ret;
}

static void reset_usb_port(void)
{
	u8 on = 1;
	u8 resp[20];
	int ret;

	ret = bpcie_icc_cmd(5, 0x10, &on, sizeof(on), resp, sizeof(resp));
	if (ret < 0)
		printk("Turn on USB failed\n");
}

static void reset_bt_wlan(void)
{
	u8 on = 3;
	u8 resp[20];
	int ret;

	ret = bpcie_icc_cmd(5, 0, &on, sizeof(on), resp, sizeof(resp));
	if (ret < 0)
...
static void do_icc_init(void)
{
	...
	bpcie_icc_cmd(2, 6, NULL, 0, reply, sizeof(reply));
	bpcie_icc_cmd(1, 0, &svc, sizeof(svc), reply, sizeof(reply));
	bpcie_icc_cmd(9, 0x20, led_config, ARRAY_SIZE(led_config),
		      reply, sizeof(reply));
}

static void icc_shutdown(void)
{
	u8 command[] = { 0, 0, 2, 0, 1, 0 };
...
static void __maybe_unused icc_reboot(void)
{
	u8 command[] = { 0, 1, 2, 0, 1, 0 };
...
{
	int ret;
	unsigned int mem_devfn = PCI_DEVFN(PCI_SLOT(sc->pdev->devfn),
					    BAIKAL_FUNC_ID_MEM);
	...
	if (!request_mem_region(sc->icc.spm_base, BPCIE_SPM_ICC_SIZE,
				"spm.icc")) {
	...
	sc->icc.spm = ioremap(sc->icc.spm_base, BPCIE_SPM_ICC_SIZE);
	if (!sc->icc.spm) {
