...
static __init unsigned long ps4_measure_tsc_freq(void)
{
	...
	emctimer_write32(EMC_TIMER_ON_OFF,
			 (emctimer_read32(EMC_TIMER_ON_OFF) & 0xFFFFFFC8) | 0x32);
	// udelay is not calibrated yet, so this is likely wildly off, but good
	// enough to work.
	udelay(300);
	emctimer_write32(EMC_TIMER_RESET_VALUE,
			 (emctimer_read32(EMC_TIMER_RESET_VALUE) & 0xFFFFFFE0) | 0x10);
